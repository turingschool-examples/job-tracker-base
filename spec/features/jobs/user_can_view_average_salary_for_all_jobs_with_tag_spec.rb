...
describe "when a user visits a specific job page" do
  scenario "they can see average salary of jobs with that tag" do
    ...
    expect(page).to have_content("#{tag1.name} - 2 (150.0)")
    expect(page).to have_content("#{tag2.name} - 2 (150.0)")
  end
end
